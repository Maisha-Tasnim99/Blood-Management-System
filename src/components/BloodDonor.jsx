...
const BloodDonor = () => {
    ...
    // Getting all the bloodDonor from the database
    const [isLoading, setIsLoading] = useState(true)
    const [bloodDonor, setBloodDonor] = useState([]);
  
    const userCollectionRef = collection(db, 'bloodDonor');
    ...
    const getUsers = async () => {
        ...
        try {
            ...
            setBloodDonor(data.docs.map((doc) => ({...doc.data(), id: doc.id}) )); 
        }
  ...
  const clearFields = (event) => {
    ...
    const createUser = (event) =>{
     ...
     const deleteUser = async (id)  => {
         ...
         const userDoc = doc(db, "bloodDonor", id)
